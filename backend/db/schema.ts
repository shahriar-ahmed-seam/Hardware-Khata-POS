/**
 * Schema as a TS string so it bundles into the Electron main process
 * (reading a .sql file via __dirname does not survive bundling).
 * Keep in sync with schema.sql (kept for reference / external tooling).
 */
...
CREATE TABLE IF NOT EXISTS stock_adjustments (
  ...
  date       TEXT NOT NULL,
  branch_id  TEXT NOT NULL REFERENCES branches(id),
  type       TEXT NOT NULL DEFAULT 'recount',
  reason     TEXT,
  created_by TEXT,
  created_at TEXT NOT NULL DEFAULT (datetime('now'))
);
...
export const FTS_SQL = String.raw`
CREATE VIRTUAL TABLE IF NOT EXISTS fts_products USING fts5(
  product_id UNINDEXED, name, sku, barcode, tokenize = 'unicode61'
);
CREATE VIRTUAL TABLE IF NOT EXISTS fts_customers USING fts5(
  customer_id UNINDEXED, name, phone, tokenize = 'unicode61'
);
CREATE VIRTUAL TABLE IF NOT EXISTS fts_suppliers USING fts5(
  supplier_id UNINDEXED, name, company, phone, tokenize = 'unicode61'
);
CREATE VIRTUAL TABLE IF NOT EXISTS fts_invoices USING fts5(
  sale_id UNINDEXED, invoice_no, customer_name, tokenize = 'unicode61'
);
`;
