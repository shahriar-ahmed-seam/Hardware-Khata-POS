...
- **Verification**: **1,161 checks** pass across eight suites (`npm run backend:verify:all`):
  all.ts 446 · api.ts 216 · run.ts 105 · e2e.ts 68 · paging.ts 91 · backup.ts 120 ·
  costing.ts 95 · mirror.ts 20 (the renderer's money math against the calculation core).
...
*can* prove is proven — 1,161 checks.
