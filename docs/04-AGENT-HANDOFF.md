...
```bash
npm install
npm run backend:verify:all   # must print 1,161 checks passed, eight suites (rebuilds for Node ABI)
...
| Suite | Checks |
|-------|-------:|
| `all.ts` (scenarios + determinism + file-DB smoke + identities) | 446 |
...
npm run backend:verify:all   # everything — eight suites, 1,161 checks
...
5. ✅ Automated end-to-end pass: 1,161 checks over eight suites.
